...
public class UtilizationGenerator {
    ...
    private static final Logger LOG = LoggerFactory.getLogger(UtilizationGenerator.class);

    private static final int create_event_source_delay = 200;
    private static final String node_id_prefix = "Utilization";

    private final UtilizationEventSourceManager eventSourceManager;
    private final DOMNotificationPublishService domPublish;
    private int eventSourceCounter;

    public UtilizationGenerator(UtilizationEventSourceManager eventSourceManager,DOMNotificationPublishService domPublish) {
     ...
     * Instances of event sources are {@link UtilizationEventSource}
     */
    public void generateEventSources(Short numberSampleEventSources, Short messageGeneratePeriod){
        try {
            ...
            for(int i = 0; i< numberSampleEventSources; i++){
                ...
                eventSourceManager.addNewEventSource(getNewEventSource(identifier, messageGeneratePeriod));
                Thread.sleep(create_event_source_delay);
            }
    ...
    private UtilizationEventSource getNewEventSource(String identifier, Short messageGeneratePeriod){
        Node sourceNode = getNewNode(identifier);
        UtilizationEventSource eventSource = new UtilizationEventSource(this.domPublish, sourceNode,  messageGeneratePeriod);
        return eventSource;
    }
