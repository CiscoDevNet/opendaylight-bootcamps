...
def clearflow():    
    for i in range(8):        
        ...
        resp, content = h.request(uri = 'http://'+controller+':8181/restconf/config/opendaylight-inventory:nodes/node/openflow:'+str(i+1)+'/table/0/',          method = 'DELETE',          headers={'Content-Type' : 'application/xml'}          )    
        return resp, content
...
#             xmlbody=xml.dom.minidom.parseString(txmlbody).toxml('UTF-8')
    ...
    #install flows in the reverse path
    switch_list.reverse()
    #How bandwidth path
    if len(switch_list)==5:
        for sw in switch_list:
            url='http://'+controller+':8181/restconf/config/opendaylight-inventory:nodes/node/openflow:'+str(sw)+'/table/0/'
            id='20'
            outputport='no'
