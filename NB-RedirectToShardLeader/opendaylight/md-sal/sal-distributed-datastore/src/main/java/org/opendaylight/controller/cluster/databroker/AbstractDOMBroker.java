        ...
        if (treeChange) {
            extensions = ImmutableMap.<Class<? extends DOMDataBrokerExtension>, DOMDataBrokerExtension>of(DOMDataTreeChangeService.class, new DOMDataTreeChangeService() {
                ...
                public <L extends DOMDataTreeChangeListener> ListenerRegistration<L> registerDataTreeChangeListener(final DOMDataTreeIdentifier treeId, final L listener) {
            		...
            		DOMShardResolutionService.class, new DOMShardResolutionService() {
                	@Override
                	public String getShardLeaderForPath(LogicalDatastoreType type, YangInstanceIdentifier path) {
                		ActorContext actorCtx = ((DistributedDataStore)datastores.get(type)).getActorContext();

                		String shardName = actorCtx.getShardStrategyFactory().getStrategy(path).findShard(path);
                		ActorRef currentRef = actorCtx.getShardManager();
                		Optional<ActorRef> leaderOpt = actorCtx.findLocalShard(shardName);
                		if (leaderOpt.isPresent()) {
                			ActorRef leaderRef = leaderOpt.get();
                		

                			if (leaderRef.compareTo(currentRef) == 0) {
                				LOG.info("Current node is leader for shard "+shardName);
                				return(null);
                			} else {
                				ActorPath actorPath = leaderRef.path();
                				Address actorAddress = actorPath.address();
                			

                				Option<String> actorHostOpt = actorAddress.host();
                				if (actorHostOpt.nonEmpty()) {
                					return(actorHostOpt.get());
                				} else {
                					LOG.info("Host of shard leader for shard "+shardName+" is empty");
                					return(null);
                				}
                			}
                		} else {
