...
import org.opendaylight.controller.md.sal.dom.api.DOMShardResolutionService;
...
public class RestconfImpl implements RestconfService {
    ...
    public NormalizedNodeContext readOperationalData(final String identifier, final UriInfo uriInfo) {
        ...
        if (mountPoint != null) {
        	...
        	DOMDataBroker dataBroker = broker.getDomDataBroker();
        	
        	
        	Map<Class<? extends DOMDataBrokerExtension>, DOMDataBrokerExtension> extMap = dataBroker.getSupportedExtensions();
        	
        	final DOMShardResolutionService shardResSvc  = (DOMShardResolutionService) extMap.get(DOMShardResolutionService.class);
        	
        	
        	String shardLeader = null;
        	
        	try {
        		shardLeader = shardResSvc.getShardLeaderForPath(LogicalDatastoreType.OPERATIONAL, normalizedII);
        	} catch (Exception e) {
        		LOG.error("Caught exception trying to get shard leader", e);
        	}
 
        	if (shardLeader == null) {
        		LOG.info("Shard leader for this transaction is current VM");
        		data = broker.readOperationalData(normalizedII);
        	} else {
        		LOG.info("Shard leader for this transaction is "+shardLeader);

        		//TODO Should return a 302
                throw new RestconfDocumentedException("Redirect to shardLeader : "+shardLeader , ErrorType.APPLICATION, ErrorTag.ACCESS_DENIED);
        	}
        }
        if(data == null) {
            final String errMsg = "Request could not be completed because the relevant data model content does not exist ";
